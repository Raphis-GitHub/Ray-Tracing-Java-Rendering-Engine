...
public class SimpleRayTracer extends RayTracerBase {
    ...
    public static boolean setLightSource(Intersection intersection, LightSource lightSource) {
        ...
        if (intersection.point == null || intersection.normal == null) {
            intersection.lightDirection = null;
            intersection.lightDotProduct = 0;
            return false;
        }
        intersection.lightDirection = lightSource.getL(intersection.point);
        intersection.lightDotProduct = intersection.normal.dotProduct(intersection.lightDirection);
        // Use Util.isZero to check if both dot products are zero
        return !(isZero(intersection.dotProduct) && isZero(intersection.lightDotProduct));
    }
}
